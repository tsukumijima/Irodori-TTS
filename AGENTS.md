...
## upstream の対応環境を維持する契約

- ローカルで実機検証できない XPU や MPS などの実装も、upstream が対応している限りそのまま維持する
- `pyproject.toml` と `uv.lock` は手元の実行環境に必要な依存関係だけを解決してよく、任意バックエンドの依存関係を省いていることを、そのバックエンドの実装を削除する根拠にしない
- upstream のマージ後はデバイス選択、精度選択、同期、キャッシュ解放、量子化の分岐を比較し、フォーク側の機能追加と無関係に欠落した対応環境を復元する
- 実機を持たないバックエンドについては動作確認済みと表現せず、ハードウェアに依存しない契約試験と upstream の実装保持を検収範囲とする

## upstream マージ後のデッドコード確認

- upstream が削除した定義をフォーク側に残す場合は、現行コードからの利用箇所か、このリポジトリで明示した公開 API 契約を必要条件とする
- マージ後は upstream で削除されたクラス、関数、定数を抽出し、利用箇所のない残存物を新しい upstream の構造に合わせて削除する
- フォーク固有の低レベル API と学習・推論機能は、upstream に存在しないことだけを理由にデッドコードと判定しない
