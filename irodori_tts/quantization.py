...
def parse_quantization_metadata(metadata: Mapping[str, str]) -> dict[str, Any] | None:
    ...
    if payload.get("format_version") != QUANTIZATION_FORMAT_VERSION:
        raise ValueError(
            f"Unsupported Irodori quantization format_version={payload.get('format_version')!r}."
        )
    ...
    if quantization_type not in QUANTIZATION_TYPES:
        raise ValueError(f"Unsupported quantization_type={quantization_type!r}.")
    return payload
...
def quantize_model(
    ...
    *,
    ...
    selected = [fqn for fqn, module in model.named_modules() if fqn and filter_fn(module, fqn)]
    if not selected:
...
def unflatten_quantized_state_dict(
    ...
    *,
    ...
    if leftover:
        raise ValueError(
            f"TorchAO safetensors deserialization left unexpected tensors: {sorted(leftover)[:8]}"
        )
    if not is_torchao_quantized_state_dict(state_dict):
